...
사용자가 캐릭터와 대화하고, 그 대화가 일기·리포트·음악 추천·저장된 대사로 이어지도록 설계하여, 단순 채팅을 넘어선 **팬 경험의 확장**을 구현하고자 했습니다.
...
- 대화 → 대사 저장 → 대화 흐름 추적 → 감정 일기 → 교환일기 → 심리 리포트로 이어지는 경험 설계
