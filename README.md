...
**High-Fidelity Persona Engine · Multi-Agent Debate Mode · Emotion-aware UI · AI Fan Experience Platform**
...
단순히 “캐릭터 이름을 붙인 챗봇”이 아니라, 각 캐릭터의 말투, 성격, 관계성, 감정 표현 방식이 대화 속에서 자연스럽게 유지되도록 설계했습니다. 사용자는 1:1 채팅, 1:2 멀티채팅, 라운드 기반 캐릭터 토론, 교환일기, 감정 일기, 심리 리포트, 음악 추천, 캐릭터 성향 지도 등 다양한 방식으로 캐릭터와 상호작용할 수 있습니다.

이 프로젝트는 특정 드라마 캐릭터에만 머무르지 않고, 향후 **역사적 인물 교육용 챗봇**, **웹툰/애니메이션 캐릭터 챗봇**, **정서 기록형 가상 에이전트** 등으로 확장 가능한 **페르소나 기반 AI 챗봇 프레임워크**를 목표로 합니다.

### Project Highlights

- 약 **2,000줄 규모의 수작업 페르소나 데이터**를 기반으로 캐릭터별 말투, 가치관, 관계성을 프롬프트 구조로 설계
- 사용자가 직접 저장한 대화만 장기 기억에 반영하는 **의도 기반 Memory Policy** 적용
- 감정 분석 결과를 UI, 일기, 리포트, 음악 추천으로 연결하는 **대화 이후 경험 설계** 구현
- 두 캐릭터의 관점 차이를 라운드별로 전개하고 사용자가 개입할 수 있는 **Multi-Agent Debate Mode** 설계
- 저장 대사를 이미지 카드로 변환해 보관·공유할 수 있도록 하여, 팬 콘텐츠와 서비스 홍보 흐름까지 고려
...
- 상황, 관계, 감정 톤에 따라 반응 방식이 달라지도록 캐릭터별 응답 규칙 설계
...
- 저장된 대사가 사용자 소장용을 넘어 공유 가능한 팬 콘텐츠이자 자연스러운 홍보 포인트가 되도록 기획
- 캐릭터가 무조건 친절하게 답하지 않고, 원작 성격에 맞게 차갑거나 신중하게 반응하도록 설계
- 캐릭터 간 관계성을 활용해 멀티채팅과 토론 모드를 구현하고, 토론에서는 주제·토론 스타일·라운드 진행·사용자 개입 흐름을 분리해 설계
...
- 현재 날짜, 요일, 시간대 정보를 자연스럽게 녹여 현실감 있는 대화 제공
- 취향을 묻는 질문과 감정 표현을 구분해 감정 UI가 과하게 반응하지 않도록 예외 처리
...
- 토론 모드, 자동 저장, 대사 저장용 자동 저장 데이터를 기억 대상에서 분리
...
저장한 캐릭터 대사를 드라마별 컬러 테마가 반영된 카드형 이미지로 변환해 갤러리에 저장하거나 공유할 수 있습니다.
단순 캡처가 아니라 작품별 분위기와 캐릭터 정보를 함께 담은 공유용 콘텐츠로 설계하여, 사용자가 인상적인 대사를 소장하는 동시에 자연스럽게 서비스 경험을 확산할 수 있도록 구현했습니다.

<img src="https://github.com/user-attachments/assets/620c2b7d-3184-4df7-b06f-54c169d47152" alt="저장 대사 이미지 카드 데모" width="300" />
...
- 약 2,000줄 규모로 직접 수집·정리한 명대사 기반 캐릭터 데이터
- 파인튜닝 없이 캐릭터 데이터를 교체·확장할 수 있는 프롬프트 기반 구조
- 장기 기억 시스템을 통한 개인화된 관계 형성

### Multi-Character Conversation & Debate Mode

- 1:1 캐릭터 채팅
- 1:2 멀티채팅
- 캐릭터 간 관계성을 반영한 상호작용
- 특정 주제에 대해 두 캐릭터가 서로 다른 관점으로 의견을 전개하는 라운드 기반 토론 모드
- 공격형, 차분형, 균형형 등 토론 스타일 선택 지원
- 사용자가 토론 중간에 의견을 제시하고 흐름에 개입할 수 있는 참여형 구조
- 토론 종료 후 라운드별 흐름과 캐릭터별 감상평을 정리하는 구조
- 토론 기록이 일반 대화 기억에 영향을 주지 않도록 대화 맥락 분리
...
- HTML2Canvas 기반으로 저장된 대사를 카드형 이미지로 변환해 갤러리에 저장하거나 공유 가능
- 사용자가 공유한 대사 카드가 자연스럽게 서비스 홍보 콘텐츠로 확장될 수 있도록 설계
- 자동 저장 데이터와 수동 저장 데이터를 분리해 보관함, 분석, 기억 목적에 맞게 관리
...
- 취향 질문을 로맨스 감정으로 오인하지 않도록 키워드 예외 처리
...
- 대화, 토론, 일기, 리포트, 음악 추천, 성향 지도 기능을 라우터 단위로 분리
- 라운드 기반 토론 진행과 일반 채팅 흐름을 분리해 대화 맥락 오염을 방지
- 라우터, AI 서비스, 데이터 모델을 분리해 기능 확장과 유지보수를 고려
...
- 데스크톱과 모바일 브라우저를 고려한 반응형 웹앱 구조

### Database

- SQLite 기반 로컬 데이터베이스
- 사용자, 대화 기록, 캐릭터 기억, 감정 일기, 교환일기, 캐릭터 성향 데이터 관리
- 자동 저장 데이터와 사용자가 직접 저장한 데이터를 구분하여 보관, 분석, 기억 목적에 맞게 활용
...
### Shareable Quote Card Flow

저장된 대사는 단순 북마크가 아니라, 대화의 맥락을 다시 확인하고 외부에 공유할 수 있는 팬 콘텐츠로 설계했습니다. 사용자가 마음에 든 대사를 저장하면 해당 대사가 포함된 전체 대화도 추적용으로 보존되며, 이후 카드형 이미지로 변환해 갤러리에 저장하거나 공유할 수 있습니다.

```mermaid
flowchart LR
  Quote[Saved Quote] --> Context[Conversation Context]
  Context --> Card[Quote Card Image]
  Card --> Save[Gallery Save]
  Card --> Share[Share / Promotion]
```
...
이 프로젝트에서 가장 큰 도전은 “AI 응답을 만드는 것”이 아니라, 사용자가 캐릭터의 말투, 감정선, 관계성을 실제로 느낄 수 있도록 **데이터를 직접 수집하고, 페르소나를 설계하고, 대화 이후의 기록·공유 경험까지 연결하는 것**이었습니다.
